...
let allLevelDiv = document.querySelectorAll('[class*= "level"]');
let twoSlices = [];
...
function shuffle(){
    ...
    for(const levelDiv of allLevelDiv){
...
function setNavLinks(){
    ...
    for(let i = 0; i< headerNavLinks.length; i++){
        ...
        link.addEventListener('click', function(){
            ...
            for(let i=0; i< allLevelDiv.length; i++){
                allLevelDiv[i].style.display = 'none';
            }
...
function checkIsComplete(){
    ...
    if(currentOrder.toString() == correctOrder.toString()){
        ...
        activeLink.classList.add('finnished');
